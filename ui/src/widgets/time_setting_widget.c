...
lv_obj_t *time_setting_create(lv_obj_t *parent, const char *text, const char *time,
  ...
  lv_obj_t *time_cont = lv_obj_create(lv_obj_0);
  lv_obj_set_flag(time_cont, LV_OBJ_FLAG_SCROLLABLE, false);
  lv_obj_add_style(time_cont, &style_time_box, 0);
  lv_obj_set_style_border_width(time_cont, 0, 0);
  lv_obj_t *label = lv_label_create(time_cont);
  lv_label_set_text(label, time);

  lv_obj_set_style_text_font(label, &opensans_regular_18, 0);
  ...
  lv_obj_set_style_text_font(lv_label_2, &opensans_bold_18, 0);
  lv_obj_add_event_cb(lv_obj_3, event_cb, LV_EVENT_CLICKED, NULL);
