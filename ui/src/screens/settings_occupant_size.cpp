...
static const lv_img_dsc_t *icons[] = {
    &icon_occupant_size_small,
    &icon_occupant_size_standard,
    &icon_occupant_size_large,
    NULL,
};

static const char *descs[] = {
    "• This information is used to more accurately detect when an\n  occupant has left the "
    "chair.\n",
    "• This information is used to more accurately detect when an\n  occupant has left the "
    "chair.\n",
    "• This information is used to more accurately detect when an\n  occupant has left the "
...
static lv_obj_t *occupant_size_selector, *label_desc;
...
static void set_occupant_size_level(lv_obj_t *obj, OCC_SIZE occupant_size);
static OCC_SIZE get_occupant_size_level(lv_obj_t *obj);
static void set_desc(OCC_SIZE occupant_size);
...
/**
 *  Resets the session data for occupant size.
 */
void clear_occupant_size_sessiondata() { session_data.occupant_size = OCC_SIZE_UNSET; }
...
static bool is_session_data_dirty() {
  ...
  if (sd->occupant_size != sd_load.occupant_size) {
    return true;
  }
  return false;
}
...
/**
 *  Updates the state of the save button based on the current settings mode and session data
 * dirtiness.
 */
static void btn_save_update_status() {
...
/**
 *  Constructs and sends the JSON-RPC request to save the occupant size.
 * param: data Pointer to the session data containing the occupant size.
 */
static void jsonrpc_request_send(session_data_t *data) {
  ...
  params["size"] = data->occupant_size;

  serializeJsonRpcRequest(0, CMD_SAVE_OCCUPANT_SIZE, params);
}

/**
 *  Sets the selected button in the radio selector based on the OCC_SIZE enum.
 * param: obj The radio selector object.
 * param: occupant_size The occupant size enum value.
 */
static void set_occupant_size_level(lv_obj_t *obj, OCC_SIZE occupant_size) {
  int8_t btn_id = occupant_size - 1;
  if (occupant_size == OCC_SIZE_UNSET) {
    btn_id = occupant_size;
  }
  radio_selector_set_selected_btn_1(obj, btn_id);
}

/**
 *  Gets the OCC_SIZE enum value from the selected button in the radio selector.
 * param: obj The radio selector object.
 * return: The current OCC_SIZE enum value.
 */
static OCC_SIZE get_occupant_size_level(lv_obj_t *obj) {
  return (OCC_SIZE)(radio_selector_get_selected_btn_1(obj) + 1);
}

/**
 *  Updates the description label based on the selected occupant size.
 * param occupant_size The occupant size enum value.
 */
static void set_desc(OCC_SIZE occupant_size) {
  if (occupant_size == OCC_SIZE_UNSET) {
  ...
  lv_label_set_text_static(label_desc, descs[occupant_size - 1]);
}
...
static void info_icon_click_cb(lv_event_t *e) {
  show_system_info_screen((char *)"settings_occupant_size");
}
...
static void btn_radio_click_cb(lv_event_t *e) {
  OCC_SIZE occ_size = get_occupant_size_level(occupant_size_selector);
  set_desc(occ_size);
  session_data.occupant_size = occ_size;
 ...
 */
static void btn_next_click_cb(lv_event_t *e) {
  send_occupant_size_session_data((OCC_SIZE)session_data.occupant_size);
...
static void set_state_ok_cb(lv_event_t *e) {
  ...
  const sys_state_t *ss = sys_state_get();
  sd_load.occupant_size = ss->occ_size;
}

/**
 *  Handler for when the screen is starting to load. 
 */
static void screen_load_cb(lv_event_t *e) {
  ...
  if (settings_mode == SETTINGS_MODE_ACTIVATION) {
    ...
    // set the occupant size level to default if unset
    if (sd->occupant_size == OCC_SIZE_UNSET) {
      sd->occupant_size = OCC_SIZE_STANDARD;
    }
  } else if (settings_mode == SETTINGS_MODE_SAVE) {
    // show the header left arrow and hide the info icon symbol
    ...
    const sys_state_t *ss = sys_state_get();
    if (ss->occ_size == OCC_SIZE_UNSET) {
      sd->occupant_size = OCC_SIZE_STANDARD;
      sd_load.occupant_size = OCC_SIZE_STANDARD;
    } else {
      sd->occupant_size = ss->occ_size;
      sd_load.occupant_size = ss->occ_size;
    }

    btn_save_update_status();
  }
  set_occupant_size_level(occupant_size_selector, sd->occupant_size);
  set_desc(sd->occupant_size);
...
//
// Subsection: UI Initialization
//

/**
 *  Initializes all LVGL objects for the Occupant Size screen.
 */
static void screen_init() {
  ...
  // Register all custom and standard event handlers
  ...
  lv_obj_set_size(cont_controls, lv_pct(100), 228);
  ...
  lv_label_set_text_static(label_subtitle, "Occupant Size");
  ...
  // add occupant size controller
  occupant_size_selector = radio_selector_create_1(cont_controls, labels, icons);
  lv_obj_set_style_pad_left(occupant_size_selector, 0, LV_PART_MAIN);
  lv_obj_set_style_pad_right(occupant_size_selector, 0, LV_PART_MAIN);
  lv_obj_add_event_cb(occupant_size_selector, btn_radio_click_cb, LV_EVENT_VALUE_CHANGED, NULL);
...
/**
 *  Gets the main screen object for Occupant Size settings, initializing it if necessary.
